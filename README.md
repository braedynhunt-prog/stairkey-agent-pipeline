...
*Under five months. ~3,700 commits, ~3,000 merged PRs, 285,000 lines of production code.
I didn't hand-write the code. This post is about the part I did build: the AI-agent
...
I'm a co-founder and the CTO of Stairkey, a business-management platform for construction
and realty companies. CRM, project management, double-entry accounting, construction
takeoff and estimating, and an offline-capable contractor portal. React and Node, SQLite,
Docker, nginx. A real product with real complexity: multi-tenant auth, money math,
offline sync, role-based permissions.
...
review, scope, security, deciding what ships. The interesting part isn't "AI wrote code,"
because everyone's AI writes code now. The interesting part is what it takes to get
...
editing the same subsystem on different branches merge cleanly and break logically.
That's a coordination problem, not a git problem.
...
harness, with screenshots and assertions on rendered state, before the PR opens. A
related hard-won rule: verify the Docker image you're testing isn't stale, because
`restart` doesn't rebuild, and we once "verified" a fix against an image that didn't
contain it.
...
2. **Verification is the product.** Generation is a commodity. Every hour I invested went
   ...
   swarms. That's where the quality came from.
...
*Braedyn Hunt is a co-founder & the CTO of Stairkey. If you're working on agentic
development tooling, or hiring people who run it in production, I'd love to talk:
